...
func setupPostgres(ctx context.Context, cfg *envconf.Config) (context.Context, error) {
	...
	if p := utils.RunCommand(fmt.Sprintf("kubectl create secret generic kflashback-db-credentials --namespace=kflashback-system --from-literal=dsn='postgres://kflashback:YOUR_PASSWORD@your-db-host:5432/kflashback?sslmode=require' kubectl apply -f %s",
